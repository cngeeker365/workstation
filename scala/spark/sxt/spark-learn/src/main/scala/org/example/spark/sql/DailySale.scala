...
object DailySale {
  def main(args: Array[String]): Unit = {
    ...
    val userSaleLogRowRDD = filteredUserSaleLogRDD.map { log => Row(log.split(",")(0), log.split(",")(1).toInt) }

    val structType = StructType(Array( StructField("date", StringType, true), StructField("sale_amount", IntegerType, true)))
